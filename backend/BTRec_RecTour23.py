...
import pandas as pd
import os
# Set DATA_DIR for file path compatibility (local or Colab)
...
def get_model_args(bert, city, epochs, pois, train_df):
    ...
    model_args.output_dir = "/content/drive/MyDrive/lakbai-colab/output_{}_e{}_{}".format(city, epochs, bert)
...
def train_bert_model(city, pois, array, epochs, model_str, use_cuda=True):
    ...
    # Print type and value of each label before conversion
    print("Label types and samples before conversion:")
    for i, val in enumerate(train_df['labels']):
    ...
    try:
        ...
        print("Error converting labels to int. Problematic values:")
        for i, val in enumerate(train_df['labels']):
            print(f"[{i}] type: {type(val)}, value: {val}")
            if i > 10:
                print("... (showing first 10)")
                break
        raise ValueError(f"Labels must be integers. Conversion failed: {e}")

    print("train_df info:")
    print(train_df.head())
    print(train_df.dtypes)
    print(f"train_df shape: {train_df.shape}")
    print(f"labels type: {type(train_df['labels'])}")
    print(f"labels sample: {train_df['labels'].head()}")
    ...
    if use_cuda and not torch.cuda.is_available():
        print("CUDA is not available. Setting use_cuda=False.")
        use_cuda = False
    import traceback
    try:
        ...
        print("Exception in train_bert_model:")
        print(err)
        traceback.print_exc()
        raise ValueError(f"train_bert_model failed: {err}")
...
def getUserLocation():
    bertlog.info("line %d, reading users info: %s", LINE(), os.path.join(DATA_DIR, "user_hometown.csv"))
    user2city = dict()
    usercity_df = pd.read_csv(os.path.join(DATA_DIR, "user_hometown.csv"),
                              sep=';',
...
def getUserCity(userid):
    if "User2City" not in setting:
        ...
        return user2city.get(userid, None)
...
def bert_train_city(city, pois, userVisits, epochs, model_str='bert', USE_CUDA=True):
    ...
    assert(model)
    # assert(0)  # Removed to allow code to continue after training
    return model,train_df
...
def test_user_preference_seqid(city, epochs, u, seqid, boot_duration, pois, user_visit, bertmodel):
    ...
    numpois = len(poi2theme.keys())
    p0,pn = int(hist_pois[0]), int(hist_pois[-1])

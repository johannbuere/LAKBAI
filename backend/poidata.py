...
def load_dataset(city, DEBUG):
  ...
  EVALPCT=20
  TESTPCT=10
  POI_file = os.path.join(DATA_DIR, "POI-{}.csv".format(city))
  ...
  allVisits_file = os.path.join(DATA_DIR, "userVisits-{}-allPOI.csv".format(city))
  ...
  if 'userid' in allVisits.columns and 'userID' not in allVisits.columns:
    allVisits['userID'] = allVisits['userid']
  ...
  for seqid in all_seqid_set:
    ...
    if len(poiids) >= 3:
      endtime = userVisits_seqid['dateTaken'].max()
      finish_time[seqid] = endtime
      finish_time_to_seqid[endtime] = seqid
    else:
  ...
  max_training_time = max(all_finish_times[0:lastindex]) if lastindex > 0 else 0
  max_eval_time = max(all_finish_times[0:lastindex2]) if lastindex2 > 0 else 0
...
def latex_table():
  ...
  print("  {{l{} }}".format(colc))
  print(r"  \hline\hline".format(colc))
  ...
  for city in allcities:
      ...
      userVisits.to_csv(os.path.join(DATA_DIR, "train_{}.csv".format(city)), sep=';')
      evalVisits.to_csv(os.path.join(DATA_DIR, "valid_{}.csv".format(city)), sep=';')
      testVisits.to_csv(os.path.join(DATA_DIR, "test_{}.csv".format(city)), sep=';')
